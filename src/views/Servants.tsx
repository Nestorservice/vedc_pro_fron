...
import { Shield, Award, TrendingUp, Users as UsersIcon } from 'lucide-react';
import { Card, Badge, Skeleton, EmptyState } from '../components/ui';
import { api } from '../services/api';
import type { Serviteur } from '../services/mockData';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

export function Servants() {
  const [serviteurs, setServiteurs] = useState<Serviteur[]>([]);
  const [stats, setStats] = useState<{ grade: string; count: number }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([api.serviteurs.list(), api.serviteurs.getStats()]).then(([s, st]) => {
      setServiteurs(s); setStats(st); setLoading(false);
    });
  }, []);
  ...
  const gradeColor = (g: string): 'success' | 'info' | 'warning' | 'danger' | 'default' => {
    const colors: Record<string, 'success' | 'info' | 'warning' | 'danger' | 'default'> = {
      'Pasteur': 'danger', 'Ancien': 'warning', 'Diacre': 'info', 'Evangliste': 'success', 'Missionnaire': 'info', 'Monitor': 'default'
    };
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => <Card key={i}><Skeleton className="h-20 w-full" /></Card>)}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800">{serviteurs.length}</p>
              <p className="text-xs text-slate-500">Serviteurs actifs</p>
            </div>
          </div>
        </Card>
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Grades distincts</p>
            </div>
          </div>
        </Card>
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800">{stats.reduce((a, b) => a + b.count, 0)}</p>
              <p className="text-xs text-slate-500">Total repertorie</p>
            </div>
          </div>
        </Card>
      </div>
            ...
            <tbody>
              {serviteurs.map(s => (
                <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all duration-200">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center">
                        <span className="text-xs font-semibold text-indigo-700">{s.personne_nom.split(' ').map(n => n[0]).join('').slice(0, 2)}</span>
                      </div>
                      <span className="text-sm font-medium text-slate-800">{s.personne_nom}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 hidden lg:table-cell"><span className="text-sm text-slate-500">{new Date(s.date_promotion).toLocaleDateString('fr-FR')}</span></td>
                  <td className="px-6 py-4"><Badge variant={s.statut === 'actif' ? 'success' : 'danger'}>{s.statut}</Badge></td>
                </tr>
              ))}
              {serviteurs.length === 0 && (
                <tr><td colSpan={5}><EmptyState title="Aucun serviteur" description="Aucun serviteur enregistre pour le moment." icon={<UsersIcon size={24} />} /></td></tr>
              )}
            </tbody>
          </table>
        </div>
